...
module.exports = (() => {
  ...
  return RequiredLibs.some((m) => !window.hasOwnProperty(m.window))
    ...
    : (([Plugin, ZLibrary]) => {
        ...
        const LibraryUsers = new (class LibraryUsers {
          ...
          #waitForT1Nlib() {
            return new Promise(async (resolve) => {
              while (
                !window.hasOwnProperty("T1Nlib") ||
                window["T1Nlib"].version !== config.info.version
              )
                await LibraryUtils.Sleep(250);
              resolve(window["T1Nlib"]);
            });
          }
          async #checkWasLibLoaded() {
            const wasLibLoaded = window.hasOwnProperty("T1Nlib");
            const isBDLoading = document.getElementById("bd-loading-icon");
            if (!wasLibLoaded && isBDLoading) return (this.plugins = new Set());
            const userPlugins = window?.T1Nlib?.LibraryUsers?.plugins;
        ...
        window.T1Nlib = new (class T1Nlib {
          constructor() {
        ...
        const { WindowInfoStore, } = LibraryModules;
        return class T1Nlib extends Plugin {
          constructor() {
            super();
